...
# Removed variables due to high concurvity: 
...
smooth_plot_df <- function(model, var, data, n = 200) {
  ...
  # identify scaled version of the variable
  scaled_var <- paste0(var, "_scale")
  
  # create prediction grid in original units
  newdat <- expand.grid(
    # create n evenly spaced values across observed range of variable
    x = seq(min(data[[var]], na.rm = TRUE),
            max(data[[var]], na.rm = TRUE),
            length.out = n),
    season = levels(data$season) # repeat those values for each season
  )
  
  names(newdat)[1] <- var # correctly rename the predictor column
  
  # scale predictor variable correctly (on scale of model)
  newdat[[scaled_var]] <-
    (newdat[[var]] - mean(data[[var]], na.rm = TRUE)) /
    sd(data[[var]], na.rm = TRUE)
    # mean and sd recreate scaled version of variable
  
  # hold other predictors at mean (0 on scaled scale) to isolate effect of variable
  for (v in c("area", "tree", "shrub", "mean_dbh")) {
    sname <- paste0(v, "_scale")
    if (!sname %in% names(newdat)) { # prevent overwriting
      newdat[[sname]] <- 0
    }
  }
  
  # generate new predictions from the GAM
  pr <- predict(
    model,
    newdata = newdat,
    type = "response", # predictions on original response scale
    se.fit = TRUE
  )
  ...
  newdat # contains plotting frame
}


# Summary data for global GAM
gam_summ <- summary(yard_global_GAM)

# Extract p-values for smoothed yard variables from global GAM
smooth_pvals <- gam_summ$s.table[, "p-value"]

# For a given predictor and season, check whether the smooth GAM effect for that 
# season is statistically significant
get_sig <- function(var, season, pvals) {
  term <- paste0("s(", var, "_scale):season", season)
  pvals[term] < 0.05
}

# Create list of variables and x-axis labels
vars <- c("area", "tree", "shrub", "mean_dbh")
xlabels <- c("Area (m²)", "Number of trees",
